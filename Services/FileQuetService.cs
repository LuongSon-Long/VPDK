        ...
        public async Task<(bool success, string message)> DeleteFileAsync(int id, int currentUserId)
{
    ...
    if (fileRecord == null) return (false, "Không tìm thấy dữ liệu.");

    // Sử dụng ID hồ sơ trực tiếp từ fileRecord
    int idHoSo = fileRecord.IddangKyDoDac ?? 0;
    ...
    catch (Exception ex)
    {
        ...
        public async Task<(byte[]? fileBytes, string? contentType, string? fileName)> ViewFileAsync(int id)
        {
            ...
            // Kiểm tra an toàn: dùng trực tiếp IddangKyDoDac từ object fileRecord
            int idHoSo = fileRecord?.IddangKyDoDac ?? 0;

            if (fileRecord == null || idHoSo == 0)
                return (null, null, null);

            // Lấy đường dẫn dựa trên ID hồ sơ lấy trực tiếp
            string folderPath = Path.Combine(_storageRoot, idHoSo.ToString());
